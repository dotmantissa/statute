...
export default function FrameworkRegistry({
  ...
  // Register framework modal state
  ...
  const handleOpenPreview = async (url: string) => {
    ...
    try {
      ...
      setPreviewData({
        url,
        status: 200,
        snippet: "Official statutory guidance registered on GenLayer. Validators fetch and evaluate this text during non deterministic consensus execution.",
      });
    } finally {
    ...
    try {
      const res = await fetch("/api/frameworks", {
        ...
        body: JSON.stringify({
          ...
          documentUrls: docUrls.length > 0 ? docUrls : ["https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32023R1114"],
  ...
  return (
    <section id="frameworks" className="py-12 scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-xs transition-all shadow-sm bg-[#26ccf0] text-[#002139] hover:bg-[#5ee1ff] hover:shadow-[0_0_20px_rgba(38,204,240,0.4)]"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {frameworks.map((fw) => {
            ...
            return (
              <div
                key={fw.framework_id}
                className="p-6 rounded-2xl border flex flex-col justify-between transition-all bg-white border-[#d2e4f0] hover:border-[#26ccf0]/50 dark:bg-[#002742] dark:border-[#003d66]"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-[#6b8699] dark:text-[#719bb5]">
                  ...
                  <h3 className="text-lg font-bold mb-1 text-[#002139] dark:text-white">
                    {fw.name}
                  </h3>

                  <div className="text-xs font-medium text-[#26ccf0] mb-3">
                    {fw.issuing_authority}
                  </div>

                  <p className="text-xs leading-relaxed mb-4 line-clamp-3 text-[#3b5a70] dark:text-[#b0d2e8]">
                    {fw.description || "Primary regulatory guidance registered on GenLayer for consensus evaluation."}
                  </p>

                  <div className="mb-4">
                    <span className="text-[11px] font-semibold uppercase tracking-wider block mb-1.5 text-[#6b8699] dark:text-[#719bb5]">
                      Jurisdiction Scope
                    </span>
                    <div className="flex flex-wrap gap-1">
                      {juris.map((j) => (
                        <span
                          key={j}
                          className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-[#f5f9fc] border border-[#d2e4f0] text-[#002139] dark:bg-[#001e33] dark:border-[#003d66] dark:text-white"
                        >
                          {j}
                        </span>
                      ))}
                ...
                <div className="pt-4 border-t border-[#d2e4f0] dark:border-[#003d66]">
                  <span className="text-[11px] font-semibold uppercase tracking-wider block mb-2 text-[#6b8699] dark:text-[#719bb5]">
                  ...
                  <div className="space-y-2">
                    {urls.map((u, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between gap-2 p-2 rounded-xl bg-[#f5f9fc] border border-[#d2e4f0] dark:bg-[#001e33] dark:border-[#003d66]"
                      >
                        <span className="text-xs font-mono truncate flex-1 text-[#3b5a70] dark:text-[#b0d2e8]">
                          {u}
                        </span>
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="p-1 rounded text-xs text-[#26ccf0] hover:underline flex items-center gap-1"
      ...
      {previewUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-2xl border shadow-2xl p-6 bg-white border-[#d2e4f0] dark:bg-[#002742] dark:border-[#003d66]">
            ...
            <div className="mb-4 p-2.5 rounded-xl text-xs font-mono truncate bg-[#f5f9fc] border border-[#d2e4f0] text-[#002139] dark:bg-[#001e33] dark:border-[#003d66] dark:text-[#26ccf0]">
              {previewUrl}
            </div>

            {previewLoading ? (
              <div className="py-16 text-center">
                <Loader2 className="w-8 h-8 animate-spin mx-auto text-[#26ccf0] mb-2" />
                <p className="text-xs text-[#3b5a70] dark:text-[#b0d2e8]">
                  Fetching primary statutory document from issuing authority...
                </p>
              </div>
            ) : (
              <div>
                <div className="flex items-center justify-between text-[11px] mb-2 text-[#6b8699] dark:text-[#719bb5]">
                  <span>HTTP Status: {previewData?.status || 200} OK</span>
                  <span>Direct Validator Source</span>
                </div>
                ...
                <div className="max-h-72 overflow-y-auto p-4 rounded-xl font-mono text-xs leading-relaxed bg-[#f5f9fc] border border-[#d2e4f0] text-[#002139] dark:bg-[#001525] dark:border-[#003d66] dark:text-[#b0d2e8]">
                  {previewData?.snippet || "Official text loaded."}
                </div>
              </div>
            )}
            ...
            <div className="mt-5 flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setPreviewUrl(null)}
                className="px-4 py-2 rounded-xl text-xs font-semibold bg-[#26ccf0] text-[#002139]"
              >
                Close Preview
              </button>
            </div>
      ...
      {showRegisterModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-xl rounded-2xl border shadow-2xl p-6 bg-white border-[#d2e4f0] dark:bg-[#002742] dark:border-[#003d66]">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
              ...
              <form onSubmit={handleRegisterSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl border text-xs font-mono outline-none focus:border-[#26ccf0] bg-[#f5f9fc] border-[#d2e4f0] text-[#002139] dark:bg-[#001e33] dark:border-[#003d66] dark:text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl border text-xs outline-none focus:border-[#26ccf0] bg-[#f5f9fc] border-[#d2e4f0] text-[#002139] dark:bg-[#001e33] dark:border-[#003d66] dark:text-white"
                    />
                  </div>
                </div>
                ...
                <div className="pt-3 flex items-center justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setShowRegisterModal(false)}
                    className="px-4 py-2 rounded-xl text-xs font-medium text-[#6b8699] hover:text-white"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={regLoading}
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-semibold bg-[#26ccf0] text-[#002139] hover:bg-[#5ee1ff] disabled:opacity-50"
                  >
